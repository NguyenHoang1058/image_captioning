...
def predict_image_attention(image_path, encoder, decoder, idx2word, transform, device):
    ...
    print("\n" + "="*50)
    print("🎯 KẾT QUẢ DỰ ĐOÁN (ATTENTION):")
    print(f"=> {caption.capitalize()}")
    print("="*50 + "\n")

    plt.figure(figsize=(8, 6))
    plt.imshow(raw_img)
    plt.title(f"{caption.capitalize()}", fontsize=14, color='green', fontweight='bold')
    plt.axis('off')
    plt.show()
...
def main():
   ...
   # Gọi các hàm load data
    ...
    # --- KHỞI TẠO MODEL ---
    print("[INFO] Đang tải trọng số Attention...")
    encoder = EncoderAttention().to(device)
    decoder = DecoderAttention(embed_size, vocab_size, 256, 2048, hidden_size).to(device)
    
    # Load trọng số 
    encoder.load_state_dict(torch.load("checkpoints/attn_encoder_epoch_5.pth", weights_only=True))
    decoder.load_state_dict(torch.load("checkpoints/attn_decoder_epoch_5.pth", weights_only=True))
    ...
    print("\n--- CÔNG CỤ DỰ ĐOÁN ATTENTION ---")
    while True:
        img_path = input("Nhập đường dẫn ảnh (hoặc gõ 'q' để thoát): ")
        if img_path.lower() == 'q':
            break
        img_path = img_path.strip('"').strip("'") 
        predict_image_attention(img_path, encoder, decoder, idx2word, transform, device)
